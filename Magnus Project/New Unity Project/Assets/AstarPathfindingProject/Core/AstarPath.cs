...
public class AstarPath : MonoBehaviour {
	...
	public static System.Version Version {
		get {
			return new System.Version (3,0,6);
		}
	}
	...
	/** Toggle to show unwalkable nodes. \todo Add variable size to unwalkable nodes, it's now hard coded to 0.3F */
	...
	public Stack<GraphUpdateObject> graphUpdateStack;
		...
		if (showUnwalkableNodes) {
			...
			for (int i=0;i<graphs.Length;i++) {
				...
				for (int j=0;j<nodes.Length;j++) {
					if (!nodes[j].walkable) {
						Gizmos.DrawCube (nodes[j].position, Vector3.one*0.3F);
					}
				}
			}
		}
	 ...
	 * \todo Add preprocessor directive for stripping out the OnGUI function
	 * \todo Disable GUILayout to reduce overhead
	 */
	public void OnGUI () {
...
#line default
	...
	/** Update all graphs within \a bounds. This function will add a GraphUpdateObject to the #graphUpdateStack. The graphs will be updated before the next path is calculated.
	 * \see Update
	 * \see DoUpdateGraphs
	 */
	...
	*/
	public void UpdateGraphs (GraphUpdateObject ob) {
		...
		if (graphUpdateStack == null) {
			graphUpdateStack = new Stack<GraphUpdateObject> ();
		}
		
		graphUpdateStack.Push (ob);
	...
	/** Updates the graphs based on the #graphUpdateStack
	 * \see UpdateGraphs
	 */
	private void DoUpdateGraphs () {
		...
		if (graphUpdateStack != null) {
			while (graphUpdateStack.Count > 0) {
				GraphUpdateObject ob = graphUpdateStack.Pop ();
